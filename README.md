...
| deployment.ingress.annotations | string | `nil` |  |
| deployment.ingress.disableSameSiteCookie | bool | `false` |  |
...
| deployment.ingress.isProxyBufferingEnabled | string | `"on"` | Enable or disable proxy buffering proxy_buffering. By default proxy buffering is disabled in the NGINX config(https://github.com/kubernetes/ingress-nginx/blob/main/docs/user-guide/nginx-configuration/annotations.md#proxy-buffering). |
| deployment.ingress.proxyBufferSize | string | `"64k"` | Sets the size of the buffer proxy_buffer_size used for reading the first part of the response received from the proxied server. By default proxy buffer size is set as "64k".(https://github.com/kubernetes/ingress-nginx/blob/main/docs/user-guide/nginx-configuration/annotations.md#proxy-buffer-size) |
| deployment.ingress.rateLimit.burstLimit | int | `150` | Nginx rate limiting: burst limit(http://nginx.org/en/docs/http/ngx_http_limit_req_module.html) |
| deployment.ingress.rateLimit.enabled | bool | `false` | Enable Nginx rate limiting |
| deployment.ingress.rateLimit.zoneName | string | `"is"` | Nginx rate limiting: zone(http://nginx.org/en/docs/http/ngx_http_limit_req_module.html) |
...
| deployment.persistence.azure.fileShare | string | `""` | Names of Azure File shares for persisted data |
| deployment.persistence.azure.secretName | string | `""` | K8s secret name for the Azure file share CI driver  |
...
| deployment.secretStore.azure.enabled | bool | `true` | Enable Azure Key Vault integration. Please note that either "Azure" or "AWS" can be enabled at a given time |
...
| deployment.secretStore.azure.nodePublishSecretRef | string | `""` | The name of the Kubernetes secret that contains the service principal credentials to access Azure Key Vault. https://azure.github.io/secrets-store-csi-driver-provider-azure/docs/configurations/identity-access-modes/service-principal-mode/#configure-service-principal-to-access-keyvault |
...
| deploymentToml.clustering.enabled | bool | `false` |  |
